...
const S={
  ...
  mfrr_opt:{l:"mFRR optimerad",c:["#34d399","#059669"],s:"mFRR opt."},
    ...
    <div style={{background:t.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans',sans-serif",color:t.tx,transition:"background 0.3s ease"}}>
      ...
      <div style={{padding:"16px 24px",maxWidth:1440,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
          ...
          <KPI label="mFRR optimerad" value={fmtE(ann.mfrr_opt)} sub={`${oc.mfrr_opt||0}/${N} optimal${dur===2?" · 2,5h BESS":dur>=4?" · båda riktn.":""}`} color={sc("mfrr_opt")} t={t}/>
        ...
        {view==="mfrr"&&(()=>{
          ...
          const cvn=months.map(m=>({label:m.label,Konventionell:m.mfrr_conv,"mFRR optimerad":m.mfrr_opt,delta:m.mfrr_opt-m.mfrr_conv}));
          const sC=cvn.reduce((s,d)=>s+d.Konventionell,0),sN=cvn.reduce((s,d)=>s+d["mFRR optimerad"],0);
          return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <Card title={`Konventionell vs mFRR optimerad — ${dur}h`} sub="Samma riktning, olika deltagande" t={t}>
              <ResponsiveContainer width="100%" height={300}><ComposedChart data={cvn} margin={{top:8,right:12,bottom:5,left:0}}>
                ...
                <Bar dataKey="mFRR optimerad" fill={sc("mfrr_opt")} opacity={0.7} radius={[2,2,0,0]}/>
              ...
              <IB color={grn} t={t}><strong style={{color:t.tx}}>Konventionell:</strong> {CH[dur]}h/dygn mFRR CM. SoC via elhandlare → pauser, ~6% obalans. {CB[dur]}h/dygn FCR-D backfill.
                <br/><br/><strong style={{color:t.tx}}>mFRR optimerad:</strong> {OH[dur]}h/dygn CM, {OE[dur]} EAM-akt/dag vs {CE[dur]}.{OB[dur]>0?` ${OB[dur]}h/dygn FCR-D backfill.`:""}
                {dur===2&&<><br/><em style={{color:amb}}>Not: 2h mFRR optimerad förutsätter ett 2,5h BESS.</em></>}
        ...
        <div style={{marginTop:20,padding:16,background:t.card,border:`1px solid ${t.bd}`,borderRadius:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16,fontSize:10,color:t.mu,lineHeight:1.7}}>
          <div><div style={{color:t.tx,fontWeight:500,marginBottom:4}}>Uthållighetsfysik</div>
            FCR-N: 1h→16h, 2h→20h, 4h→24h<br/>FCR-D: oberoende av uthållighet<br/>FCR-N+D: 0.5 MW vardera<br/>mFRR opt 2h: kräver 2,5h BESS<br/>mFRR opt 4h: båda riktningar</div>
