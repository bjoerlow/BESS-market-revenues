...
function genSyntheticArea(){
  ...
  while(d<end){
    ...
    DURATIONS.forEach(dur=>{
      ...
      const id_cm=hasMfrr?bestCM*hours:0;
      const id_eam=hasMfrr&&bestEAM>0?ID_EAM[dur]*act*bestEAM*days*rte:0;
      const id_cost=hasMfrr&&bestEAM>0?ID_EAM[dur]*act*spread*days*0.5:0;
      ...
      const all={fcrn:v_fcrn,fcrd:v_fcrd,mfrr_conv:v_conv,mfrr_id:v_id,intraday1:v_id1,intraday2:v_id2,dayahead:v_da};
      ...
      <div style={{padding:"14px 20px",maxWidth:1440,margin:"0 auto"}}>
          ...
          return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card title={`Konventionell vs mFRR+intradag — ${duration}h`} sub="Samma riktning, olika återställningsmetod">
              ...
              <InfoBox color={T.green}><strong style={{color:T.text}}>Konventionell:</strong> {CONV_H[duration]}h/dygn mFRR CM. SoC via elhandlare → pauser, ~6% obalans, resterande timmar FCR-D.
                <br/><br/><strong style={{color:T.text}}>mFRR+intradag:</strong> 24/7 CM. SoC via intradagsmarknaden → inga pauser, {ID_EAM[duration]} EAM-akt/dag vs {CONV_EAM[duration]}.</InfoBox></Card></div>);})()}
        ...
        <div style={{marginTop:16,padding:14,background:T.card,border:`1px solid ${T.border}`,borderRadius:10,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14,fontSize:10,color:T.textMuted,lineHeight:1.7}}>
          <div><div style={{color:T.text,fontWeight:600,marginBottom:3}}>Uthållighetsfysik</div>
            FCR-N: 1h→16h, 2h→20h, 4h→24h<br/>FCR-D: oberoende av uthållighet<br/>Intradag/DA: effektiv MWh 1.0/1.5/1.8 (avtagande)<br/>Kalibrerat mot Clean Horizon Storage Index</div>
          <div><div style={{color:T.text,fontWeight:600,marginBottom:3}}>mFRR riktning</div>
            SE1/SE2: ned bättre (överskott norr)<br/>SE3/SE4: upp bättre (underskott söder)<br/>Auto-valt per månad baserat på priser</div>
          ...
          <div><div style={{color:T.text,fontWeight:600,marginBottom:3}}>Beräkning</div>
            7 strategier × 3 uthålligheter<br/>RTE: 85% · FCR-D dual: 87%<br/>Intradag: 50%/30% capture<br/>DA: 45% capture, ~8% obalans</div>
        </div>
      </div>
    </div>
  );
