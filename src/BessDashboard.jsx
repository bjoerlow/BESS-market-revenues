...
const FH={1:16,2:20,4:24},IE={1:3,2:5,4:8},CH={1:12,2:16,4:20},CB={1:12,2:8,4:4},CE={1:2,2:3,4:5};
const IH={1:16,2:24,4:24},EM={1:1.0,2:1.8,4:3.2};
...
function genArea(){
  ...
  while(d<end){
    ...
    DURS.forEach(dur=>{
      ...
      const ih=IH[dur];
      const icm=has?(dur>=4?(cu+cd)*ih*days:bC*ih*days):0;
      const iep=dur>=4?(eu+ed)/2:bE;
      const ieam=has&&iep>0?IE[dur]*act*iep*days*rte:0;
      const icost=ieam>0?IE[dur]*act*sp*days*0.5:0;
    ...
    <div style={{background:t.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans',sans-serif",color:t.tx,transition:"background 0.3s ease"}}>
      ...
      <div style={{padding:"16px 24px",maxWidth:1440,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
          ...
          <KPI label="mFRR+intradag" value={fmtE(ann.mfrr_id)} sub={`${oc.mfrr_id||0}/12 optimal${dur===2?" · 2,5h BESS":dur>=4?" · båda riktn.":""}`} color={sc("mfrr_id")} t={t}/>
        ...
        {view==="services"&&(()=>{
          ...
          const sD=months.map(m=>{const d=m.ym?new Date(+m.ym.slice(0,4),+m.ym.slice(5,7),0).getDate():30;
            return{label:m.label,"FCR-N":Math.round(m.fcr_n_price*0.5*FH[dur]*d*mw),
              ...
              "mFRR CM":Math.round(m.mfrr_cm_up_price*d*24*mw),"mFRR EAM":Math.round(IE[dur]*0.25*m.mfrr_eam_up_price*d*0.85*mw),
          ...
          return(<div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <Card title={`Konventionell vs mFRR+intradag — ${dur}h`} sub="Samma riktning, olika återställningsmetod" t={t}>
              <ResponsiveContainer width="100%" height={300}><ComposedChart data={cvn} margin={{top:8,right:12,bottom:5,left:0}}>
                ...
                <Line dataKey="delta" name="Δ ID−konv." stroke={t.tx} strokeWidth={1.5} strokeDasharray="4 2" dot={{r:2,fill:t.tx}}/>
              </ComposedChart></ResponsiveContainer>
              ...
              <IB color={grn} t={t}><strong style={{color:t.tx}}>Konventionell:</strong> {CH[dur]}h/dygn mFRR CM. SoC via elhandlare → pauser, ~6% obalans. Resterande {CB[dur]}h/dygn FCR-D backfill.
                <br/><br/><strong style={{color:t.tx}}>mFRR+intradag:</strong> {IH[dur]}h/dygn CM. SoC via intradagsmarknaden → {IE[dur]} EAM-akt/dag vs {CE[dur]}.
        ...
        {view==="dayahead"&&(()=>{
          ...
          return(<Card title={`Day-ahead arbitrage — ${dur}h (${mw*dur} MWh)`} sub="85% capture · Obalanskostnad highlightad" t={t}>
          ...
          <div style={{marginTop:14,display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {DURS.map(d=>{const a=durC.slice(-12).reduce((s,r)=>s+(r[`opt_${d}h`]||0),0);
              return(<div key={d} style={{background:t.cA,border:`1px solid ${t.bd}`,borderRadius:10,padding:14,textAlign:"center"}}>
                ...
                <div style={{fontSize:10,color:t.mu}}>Optimal 12 mån</div></div>);})}</div></Card>)}
          ...
          <div style={{marginTop:14,display:"flex",gap:6,flexWrap:"wrap"}}>
            {SIDS.map(sid=>{const cnt=months.filter(m=>m.optimalStrategy===sid).length;if(!cnt)return null;
              return(<div key={sid} style={{background:sc(sid)+"12",border:`1px solid ${sc(sid)}30`,borderRadius:6,padding:"5px 10px",display:"flex",alignItems:"center",gap:6,fontSize:11}}>
                ...
                <span style={{fontWeight:500,fontFamily:"'JetBrains Mono'",color:sc(sid)}}>{cnt} mån</span></div>);})}</div>
        ...
        <div style={{marginTop:20,padding:16,background:t.card,border:`1px solid ${t.bd}`,borderRadius:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16,fontSize:10,color:t.mu,lineHeight:1.7}}>
          ...
          <div><div style={{color:t.tx,fontWeight:500,marginBottom:4}}>mFRR deltagande</div>
            Konv: 1h→12h, 2h→16h, 4h→20h/dygn<br/>+ID: 1h→16h, 2h→24h, 4h→24h (båda)<br/>4h: bud på BÅDE upp OCH ned</div>
